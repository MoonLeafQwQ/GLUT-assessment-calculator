...
在信息化的今天，桂工的学生甚至还在手动计算自己的综测分数，在我受够了那些乱七八糟的计算公式之后，就有了这个项目。但由于旧版本项目在今天以及不适合新版的综测计算，故重新翻新了一下

## 介绍

这是一个适用于**桂林理工大学**的综测计算器，用它可以来帮助你快速计算综测，填入你的各项成绩、加分、扣分，按照综测计算规则会自动计算结果，并实时反馈出来

使用React + Antd开发，目前只有基于Electron开发的桌面版，网页版还在开发中，之后重置版本会考虑开发网页版

## 关于重置

偶然见到群友发了这个项目的链接，想上手使用发现适配的表格以及填写过于久远，所以烧了token翻新了软件*添加了许多BUG*，因为本人（该分支作者）非计科出身，所以不能保证之后能有很好的产出或者更新，请多海涵TwT
此重制版更新了：

- 新版的Excel表格适配，目前仅适配了26年地院的表格导入和填写，之后考虑使用关键词或手动匹配
- 添加了自定义公式，妈妈再也不用担心美育基础分要不要*70%了！
- 让界面能适应窗口化，随意拉伸
<img width="1280" height="720" alt="2026-09-09 00-12-16" src="https://github.com/user-attachments/assets/869eb0eb-38b4-4956-b02f-203a64344130" />
...
原作者本人为信院学生，考虑到不同学院模块分数占比可能有区别，所以在分数占比这块是可以调整的
